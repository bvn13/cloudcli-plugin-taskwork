...
function ok(body: ResponseBody): RpcResponse {
  return { status: 200, body };
}

/** Path parameters are decoded, then hard-validated: `../` never reaches a handler. */
function decodeSegment(raw: string, field: string): string {
  let decoded: string;
  try {
  ...
  try {
    ...
    if (segments[0] === 'tasks') {
      ...
      if (segments.length >= 2) {
        const taskId = decodeSegment(segments[1] as string, 'taskId');
        ...
        if (segments[2] === 'attachments') {
          ...
          if (segments.length === 4) {
            const projectId = decodeSegment(segments[3] as string, 'projectId');
