...
export class Repository {
    ...
    createTask(title) {
        return this.mutate((store) => {
            ...
            while (store.tasks.some((task) => task.id === id))
                id = generateTaskId(now.getTime()); // INV-1
            const task = { id, title, createdAt: iso, updatedAt: iso, attachments: [] };
    ...
    attachProject(taskId, projectId, projectName) {
        return this.mutate((store) => {
            ...
            const holder = store.tasks.find((candidate) => candidate.attachments.some((a) => a.projectId === projectId));
            if (holder)
                throw new ConflictError('Project is attached to another task.');
            const attachment = {
                projectId,
                projectName,
    ...
    detachProject(taskId, projectId) {
        return this.mutate((store) => {
            ...
            if (index === -1)
                throw new NotFoundError(`Project ${projectId} is not attached to ${taskId}.`);
            task.attachments.splice(index, 1);
            task.updatedAt = new Date().toISOString();
            return task;
        });
    }
    setAttachmentSession(taskId, projectId, sessionId) {
        return this.mutate((store) => {
            const task = this.find(store, taskId);
